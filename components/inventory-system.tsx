...
// Helper function to generate a formatted timestamp for new transactions
...
export default function InventorySystem() {
  ...
  // Use the inventory data hook instead of local state
  const {
    inventory,
    transactions,
    addTransaction,
  ...
  // New state variables for sorting and searching
  ...
  // Add a new state for inventory item editing
  ...
  const handleDeleteTransaction = async () => {
    ...
    // Remove the transaction and add the deletion notification
  ...
  const handleAddNewItem = async () => {
    ...
    // Add to item definitions if it's not there
  ...
  const handleSaveInventoryEdit = async () => {
    ...
    // Create a transaction record for the edit
    const transaction: Transaction = {
      id: Date.now().toString(),
      itemType: originalName, // Use original name to reference the item
      ...
      notes: nameChanged
        ? `Item renamed from "${originalName}" to "${name}"${unitChanged ? ", unit changed" : ""}${
            quantityDifference !== 0 ? ", quantity adjusted" : ""
          }`
        : unitChanged
    ...
    // If name changed, update item definitions
    if (nameChanged) {
  ...
  // Toggle inventory sort order
  const toggleInventorySort = () => {
    if (inventorySortOrder === null) {
      setInventorySortOrder("asc")
    } else if (inventorySortOrder === "asc") {
      setInventorySortOrder("desc")
    } else {
      setInventorySortOrder(null)
    }
  }

  // Toggle transaction sort order
  const toggleTransactionSort = () => {
    if (transactionSortOrder === null) {
      setTransactionSortOrder("asc")
    } else if (transactionSortOrder === "asc") {
      setTransactionSortOrder("desc")
    } else {
      setTransactionSortOrder(null)
    }
  }

  // Export inventory to CSV
  const exportInventoryToCSV = () => {
    // Calculate inventory values using FIFO method
    ...
    // Create CSV header with Value column
  ...
  // Filter and sort inventory items - show all items for both admin and regular users
  const filteredAndSortedInventory = (() => {
    // Show all items from itemDefinitions for both admin and regular users
  ...
  // Get recent transactions (last 24 hours) for non-admin users
  ...
  // Handle rebuilding inventory from transactions
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                  ...
                  <div className="border-t border-gray-200 pt-5">
                    <div className="grid grid-cols-1 gap-4">
                      {(() => {
                        // Calculate inventory values for display
                        const itemValues: Record<string, { quantity: number; totalValue: number; avgPrice: number }> =
                          {}
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                {/* Show edit and delete buttons for admin or KAB123 user */}
