...
export async function getAllInventoryItems(includeZeroQuantity = true): Promise<InventoryItem[]> {
  return safeRedisOperation(
    async () => {
      ...
      const completeItems = Object.values(items).filter(
        (item): item is InventoryItem =>
          item.name !== undefined && item.quantity !== undefined && item.unit !== undefined,
      )

      // Return all items or only those with quantity > 0
      return includeZeroQuantity ? completeItems : completeItems.filter((item) => item.quantity > 0)
    },
    [], // Return empty array as fallback
  )
}
